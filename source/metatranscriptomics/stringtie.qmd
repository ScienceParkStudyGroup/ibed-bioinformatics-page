...
If you want to install it yourself, the command below installs the version verified on Crunchomics. Before running it, check the tool's [GitHub](https://github.com/gpertea/stringtie) for a newer version and swap the version number if one is available. We install via bioconda rather than building from source, since bioconda has kept pace with the latest releases and building from source ran into `make` issues.
