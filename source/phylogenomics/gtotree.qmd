...
Phylogenomics is a complex task and there are many different ways to generate a phylogenetic tree. Among others, you should think about:

-  How many genomes you want/have to include
-  What markers to should use
-  What the best phylogenetic model for your data is

While not extensive @sec-considerations can give you some pointers to get started.
...
If you want to install it yourself, the command below installs the version verified on Crunchomics. Before running it, check the [GToTree wiki](https://github.com/AstrobioMike/GToTree/wiki/) for a newer version and swap the version number if one is available.
...
### Important considerations {#sec-considerations}

#### Choosing the right taxon set

The GToTree helper script `gtt-get-accessions-from-GTDB` should be useful to get you started with selecting reference genomes to start with. However, there might be cases where you run into problems, for example, if your taxonomic group of interest has too many genomes then you could consider to:

- Subset your selection further by only [keeping one representative per order](https://github.com/AstrobioMike/GToTree/wiki/things-to-consider#subsetting-the-GTDB-further)
- If that leaves too few genomes you could consider running GToTree with the full set using the FastTree option, manually down-sample the tree and then rerun the tree with a better model on the downsampled set

If you want to generate a tree of life with representatives from both the archaea and bacteria you can check this code [here](https://github.com/AstrobioMike/GToTree/wiki/example-usage#tol-example). Just keep in mind that by adding filters, such as only working with representative genomes might omit genomes from candidate phyla.
...
#### Phylogenetic models in IQ-TREE
