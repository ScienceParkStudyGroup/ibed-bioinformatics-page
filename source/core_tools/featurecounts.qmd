...
If you want to install it yourself, the command below installs the version we tested. Before running it, check the [FeatureCounts documentation](https://subread.sourceforge.net/featureCounts.html) for a newer version and swap the version number if one is available.
