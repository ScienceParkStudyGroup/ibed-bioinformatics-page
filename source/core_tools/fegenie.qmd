...
Additionally the script itself was modified to fix an issue with the gbk parsing by creating `FeGenie_gbk.py`, a modified copy of FeGenie.py specifically designed to fix the GBK parsing bug. This change was needed since the previous nested defaultdict caused incorrect ORF outputs.
...
Open the `$FEGENIE_DIR/FeGenie_gbk.py` and find the section starting with `f args.gbk:` and replace this section until `print("\n")` with:
...
python /zfs/omics/projects/bioinformatics/scripts/fegenie_fix_prokka_gbk.py --fasta_dir data/prokka --gbk_dir data/prokka --output_dir data/prokka_fixed
...
FeGenie_gbk.py -bin_dir data/prokka_fixed/ -bin_ext gbk -out fegenie_out/gbk --gbk
```
