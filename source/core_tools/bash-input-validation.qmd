...
Note: When writing this yourself a common error is to not add spaces here `[ -f` and here ` ]`. This is an important part of the syntax, which when not added will create an error.
...
Since for this part you often feed forward and reverse reads as input files, this is a good usage case of using the OR keyword `||` and we will use the `break` keyword to stop the script if an issue occurs.
