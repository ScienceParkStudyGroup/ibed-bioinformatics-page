...
- Several versions are available as part of the bioinformatics share:
  -   GTDB_Tk v2.7.1 and the [GTDB v232 database](https://gtdb.ecogenomic.org/stats/r232)
  -   GTDB_Tk v2.6.1 and the [GTDB v226 database](https://gtdb.ecogenomic.org/stats/r226)
  -   GTDB_Tk v2.4.0 and the [GTDB v220 database](https://gtdb.ecogenomic.org/stats/r220)
-   If you have access to crunchomics and have not yet access to the bioinformatics you can send an email with your Uva netID to [Nina Dombrowski](n.dombrowski@uva.nl).
...
<details>
  <summary>GTDB v2.4.0 with r220</summary>
...
<details>
  <summary>GTDB v2.6.1 with r226</summary>
...
<details>
  <summary>GTDB v2.7.1 with r232</summary>

```{bash}
# Setup GTDBtk
mamba create -n gtdbtk_2.7.1 -c conda-forge -c bioconda gtdbtk=2.7.1

# Download gtdb data, v232
cd <my_database_folder>
wget https://data.gtdb.ecogenomic.org/releases/latest/auxillary_files/gtdbtk_package/full_package/gtdbtk_data.tar.gz
tar xvzf gtdbtk_data.tar.gz 

# Link database to gtdbtk
conda activate gtdbtk_2.7.1
conda env config vars set GTDBTK_DATA_PATH="<my_database_folder>/gtdb/release232";
conda deactivate
```
...
# Ensure the right database is loaded 
...
# Ensure the right database is loaded  (should be 262)
echo $GTDBTK_DATA_PATH
...
# Ensure the right database is loaded (should be 232)
echo $GTDBTK_DATA_PATH
...
gtdbtk classify_wf --genome_dir  genome_dir/ \
  ...
  # place species in pplacer trees even if they are classified with skani
  --place_species \
  --cpus 20
...
Note, that the first step of the `classify_wf` workflow is an ANI pre-screening step. If all genomes have been classified by the ANI screening step, Identify and Align steps will be skipped. If you still want to run these steps, you can add the `--place_species` argument to your command.
...
-   **Issue 1:** Running out of memory as described [here](https://ecogenomics.github.io/GTDBTk/faq.html)
    -   Solution 1: For Crunchomics users: Use at least 150G of memory (and make sure to submit the job via srun or sbatch)
