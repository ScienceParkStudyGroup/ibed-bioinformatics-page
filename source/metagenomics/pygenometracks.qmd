...
If you want to install it yourself, the command below installs the version verified on Crunchomics. Before running it, check the tool's [GitHub releases page](https://github.com/deeptools/pyGenomeTracks/releases) for a newer version and swap the version number if one is available.
