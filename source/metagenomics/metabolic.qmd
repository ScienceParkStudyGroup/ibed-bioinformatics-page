...
#print the help information (and ensure everything is running ok)
perl METABOLIC-C.pl -help

#prepare the example data that is available in the METABOLIC directory
mkdir results/fna
mkdir results/faa
mkdir data/reads
...
#get contigs
cp /zfs/omics/projects/bioinformatics/software/metabolic/v4.0.0/METABOLIC_test_files/Guaymas_Basin_genome_files/*fasta results/fna/

#get proteins
cp /zfs/omics/projects/bioinformatics/software/metabolic/v4.0.0/METABOLIC_test_files/Guaymas_Basin_genome_files/*faa results/faa/

#for proteins, ensure that there is no space in the header
#if there is, we remove everything after the space as follows:
for i in results/faa/*faa; do
    cut -f1 -d " " "$i" > "$i.tmp" && mv "$i.tmp" "$i"
done

#for proteins, ensure that we don't have duplicate protein IDs, this code should return nothing
cat results/faa/*faa | grep ">" | sort | uniq -d

#get reads
#since the files are larger, we generate a symbolic link
ln -s /zfs/omics/projects/bioinformatics/software/metabolic/v4.0.
0/METABOLIC_test_files/METABOLIC_test_reads/SRR3577362_sub_*fastq data/reads/

#get file with path to the reads 
echo "#Read pairs:" > read_location
find "$(pwd)/data/reads" -name "*fastq.gz" >> read_location
```

Run the G mode with fasta files as input:
```{bash}
perl /home/ndombro/personal/software/METABOLIC_4.0/METABOLIC/METABOLIC-G.pl \
  -in-gn results/fna/ \
  -o results/metabolic/v1_g_mode_w_fasta
```
...
perl /home/ndombro/personal/software/METABOLIC_4.0/METABOLIC/METABOLIC-G.pl \
  -in results/faa/ \
  -o results/metabolic/v2_g_mode_w_faa
```

Run the C mode, **notice that this mode still seems to be buggy and I recommend to either install Metabolic yourself if you want to use it or start with using the G mode**:

```{bash}
perl /home/ndombro/personal/software/METABOLIC_4.0/METABOLIC/METABOLIC-C.pl \
  -in-gn results/fna \
  -st nanopore \
  -r read_location \
  -o results/metabolic/v3_c_mode_w_faa > error.log 2>&1
```
