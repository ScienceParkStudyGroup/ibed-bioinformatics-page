...
-   Identify and separate both the ITS1 and 18S rRNA gene using the Uniprot and Silva database, respectively
...
cp /zfs/omics/projects/amplicomics/bin/NanoITS_0.2/config/config.yaml .
```
...
You can select what markers you want to analyse in `markers: ["SSU", "ITS1"]`. The workflow was developed for primers targeting both the SSU and ITS1 but the workflow will also run for either option selected and we plan to in the future extend the workflow to also accept the LSU and ITS2.
...
cp /zfs/omics/projects/amplicomics/bin/NanoITS_0.2/jobscript.sh .
```
...
-   Runs NanoITS. When using the script as is, the script assumes that NanoITS is installed at this path `/zfs/omics/projects/amplicomics/bin/NanoITS_0.2` and that the config.yaml is located in the folder from which you analyse the data. If that is not the case, change the paths accordingly
