...
cp /zfs/omics/projects/amplicomics/bin/NanoITS_0.3/config/config.yaml .
```
...
cp /zfs/omics/projects/amplicomics/bin/NanoITS_0.3/jobscript.sh .
```
...
-   Runs NanoITS. When using the script as is, the script assumes that NanoITS is installed at this path `/zfs/omics/projects/amplicomics/bin/NanoITS_0.3` and that the config.yaml is located in the folder from which you analyse the data. If that is not the case, change the paths accordingly
