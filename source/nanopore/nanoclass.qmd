...
-   Subsample the fastq files to only work with 100 reads per sample. 
    ...
    -   **Important:** Once you decided on a final classifier or if you only want to run a single classifier you want to turn subsampling off by setting `skip:true` in order to run NanoClass2 on all reads.
-   Run NanoClass2 using all 11 classifiers. If you already have a favorite classifier you can only select the tool that you want to use. We have good experiences with running minimap2 and kraken2.
...
-   Requests 20 crunchomics cpus and 100G of memory. One can set this lower, but for testing purposes you might keep these values. The can be changed with `#SBATCH --cpus-per-task=20` and `#SBATCH --mem=100G`
